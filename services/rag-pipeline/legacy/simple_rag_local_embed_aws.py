...
text_splitter = RecursiveCharacterTextSplitter(
    chunk_size=2000,       # Target chunk size (larger than before to fit full articles)
    chunk_overlap=200,     # Slight overlap to maintain context between splits if needed
    separators=["(?=Стаття)"], # Split before "Стаття", then by paragraphs/lines if needed
    is_separator_regex=True
)
documents_chunks = text_splitter.split_text(document)

for i in  range(len(documents_chunks)):
    if len(documents_chunks[i]) > 10000:
        documents_chunks[i] = documents_chunks[i][:10000]
